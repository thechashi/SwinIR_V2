...
def main():
    ...
    for idx, path in enumerate(sorted(glob.glob(os.path.join(folder, '*')))):
        ...
        output = output*(0.33*args.abs_max)
...
def get_image_pair(args, path):
    ...
    if args.task in ['classical_sr', 'lightweight_sr']:
        if 'npz' in imgext:
            img_gt = unpz.get_npz(path)
            img_gt = img_gt/(0.33*args.abs_max)
            img_gt = img_gt.reshape((img_gt.shape[0], img_gt.shape[1], 1))
            
            img_lq = unpz.get_npz(f'{args.folder_lq}/{imgname}x{args.scale}{imgext}')
            img_lq = img_lq/(0.33*args.abs_max)
